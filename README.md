...
- [ ] **Phase 1 — 권한관리 + 회원관리**: 로그인/JWT/RBAC, BranchScopeGuard, 회원 CRUD ([1-2](docs/1.spec/10_인사조직/1-2_권한관리.md), [1-6](docs/1.spec/20_이용자서비스/1-6_회원관리.md)) — API는 거의 완료(로그인·토큰갱신·Role전환·회원 등록/수정/상태전환), **admin-web에 등록·수정 화면과 수강/PT세션 조회가 아직 없어 데모 기준 미완료**

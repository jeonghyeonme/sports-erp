...
- **게시된 덱:** https://claude.ai/artifact/GBiFad42sswiYAs82BUqLU (claude.ai 계정 소유, 비공개). 이 저장소에는 덱 자체가 아니라 덱을 만드는 소스가 있다.
- 슬라이드 구성안은 상위 문서 `4-3_3주차발표_슬라이드구성.md`를 본다. 현재 덱은 그 구성안 이후 RFP 산출물 3종 + 도메인 9개(도식 1장 + 구현 화면 1~2장) 구조로 다시 짰으므로, 구성안 문서와 덱이 어긋난다.

...
section('deliverables', { notes: '원본 RFP가 요구한 것은 세 가지다(기업 분석 자료, 개발 작업계획서, ERP 앱). 발표 순서는 분석 → 계획 → 구현이다. 주의: 기업 분석 자료는 RFP상 3개 파트(현황 분석, 도입 후 기대효과 분석, 유사기업 매뉴얼 분석)인데 현황 분석 파트만 작성했고 나머지 둘은 근거와 함께 의도적으로 제외했다(2-3 추적표 §1-3). 슬라이드에 이 사실을 그대로 적어 두었다.' },
...
// ── 3 : 기업 분석 자료 ─────────────────────────────────────────────
section('d2-doc', { notes: '기업 분석 자료(5-1). 원본 RFP가 시스템 설계에 앞서 "경영시스템·관리시스템 분석"을 선행 과업으로 요구했다. 6개 영역(사업 구조, 위탁계약 라이프사이클, 인력 운용, 예약·결제·정산, 물적자원관리, 문서관리)을 각각 실무 상세 → 분석 결과 → 결론으로 정리했고, 영역 하나만 읽어도 완결된다. 왼쪽은 문서의 실제 화면. 아래 한 줄은 범위 명시: RFP상 기업 분석 자료는 3개 파트인데 현황 분석 파트를 작성했고, 기대효과·유사기업 분석은 근거(실제 매출·계약 데이터가 없는 포트폴리오 프로젝트, 벤치마킹 대상 접근 불가)와 함께 제외했다. 분석의 근거는 원본 RFP와 일반적인 실무 관행을 조사해 재구성한 것이며 스포이즘의 내부 자료가 아니다.' },
...
section('d2-trace', { notes: '이 발표의 핵심 슬라이드(1분 30초). 분석에서 발견한 공백이 설계 결정과 구현으로 이어졌다는 증거다. 제외·미구현 행은 슬라이드에 넣지 않았다. 문장: "분석서에는 실무 전체를 명시하되, 구현을 진행하면서 불필요하다고 판단한 것은 제외하는 방향으로 진행했다." 부가세 행은 백업이며 질문이 나오면 구성안 §5-7로 답한다.' },
...
const domData = [
  { id: 'dom-auth', name: '권한관리', pill: '구현됨', fig: D.dAuth, rfp: '관리자 계정에만 권한 부여 · 본사·현장·이용자 권한 분리 · 퇴사 후 접근 통제', view: '세부 권한 대신 <b>4개 역할</b>로 고정하고, 지점 간 데이터는 항상 격리한다', feats: ['역할별 화면·API 접근 제어', '다른 지점 데이터 접근 차단', '퇴사·탈퇴 즉시 접근 차단', '본사의 직원 역할 전환'], src: '1-2 권한관리', notes: '권한관리: 4개 역할과 3관문. 요청은 로그인 → 역할 → 지점의 세 관문을 통과해야 데이터에 닿고, 다른 지점의 데이터는 마지막 관문에서 막힌다. 지점 격리는 가드와 컨트롤러가 이중으로 검사한다. 질문이 나오면(구성안 §5-6): 자동 테스트로 확인했고 그 과정에서 검사가 빠져 있던 라우트 1건을 찾아 수정했다.' },
  { id: 'dom-hr', name: '인사정보관리', pill: '구현됨', fig: D.dHr, rfp: '현장별 인사 관리 · 모든 인사 기록을 한 화면에서 · 권한 부여·승인 시 수정', view: '직원은 지점이 채용하는 게 아니라 <b>본사가 채용해 파견</b>한다', feats: ['채용·재배치는 본사 전용 (API)', '파견 이력을 기록으로 보존', '지점 관리자는 소속 직원 관리', '직원 본인 정보 조회'], src: '1-3 인사정보관리', notes: '인사정보관리: 본사가 채용하고 지점에 파견한다. 재배치도 본사만 할 수 있고 이전 파견은 기록으로 남는다. 지점 관리자는 현재 파견된 직원의 일상 관리(정보 수정, 퇴사 처리)만 한다. 이 구조는 기업 분석에서 발견한 것이다. 주의: 채용·재배치는 API와 통합 테스트로만 구현했고 admin-web에는 화면이 없다("화면에서 채용한다"고 말하지 말 것).' },
  { id: 'dom-attend', name: '근태관리', pill: '구현됨', fig: D.dAttend, rfp: '현장별·개인별 근태 · 휴무·연차·병가 일수 관리 · 업무일지', view: '수동 체크인에 <b>지각을 자동 판정</b>하고, 연차만 잔여일수를 차감한다', feats: ['출근·퇴근 체크', '지각 자동 판정(기준시각 +10분)', '휴가 신청·승인·반려', '연차 잔여일수 · 업무일지'], src: '1-4 근태관리', notes: '근태관리: 체크인은 지점 기준시각 +10분을 넘기면 지각이다. 휴가는 신청 → 지점 관리자 승인이고, 연차만 잔여일수에서 차감한다. 병가·경조는 승인돼도 차감하지 않는다. 시연: 체크인 → 휴가 신청 → 승인 → 잔여연차 감소. 주의: 조퇴·결근·휴가 상태는 자동 판정이 없으므로 "근태 상태 5종"이라고 말하지 말 것.' },
  { id: 'dom-board', name: '게시판·공지사항', pill: '구현됨', fig: D.dBoard, rfp: '본사→현장, 현장→이용자 게시판 분리 · 직원 교육자료', view: '공지를 <b>본사 공지와 지점 공지</b>로 나눠 열람 범위를 다르게 한다', feats: ['공지 작성·수정·삭제', '전체 또는 지정 지점 대상', '자기 지점 글만 열람', '수정은 작성자 본인만'], src: '1-5 게시판·공지사항', notes: '게시판: 본사 공지는 전체 또는 지정한 지점에, 지점 공지는 자기 지점의 회원·직원에게 간다. 지점 사용자는 전체 공지와 자기 지점의 글만 볼 수 있다. 시연: 본사·지점 공지 작성, 작성자 본인만 수정.' },
  { id: 'dom-member', name: '회원관리', pill: '구현됨', fig: D.dMember, rfp: '지점별 회원 인적사항 · 수강·결제 내역 · 타 현장 조회 불가', view: '회원은 항상 <b>한 지점에 속하고</b>, 그 지점 관리자만 다룬다', feats: ['회원 등록·수정', '상태 관리(활성·휴면·탈퇴)', '미성년 보호자 동의 확인', '담당 직원 배정'], src: '1-6 회원관리', notes: '회원관리: 지점 관리자가 자기 지점에 회원을 등록하고, 미성년이면 보호자 동의를 확인한다. 다른 지점의 회원은 조회·수정할 수 없다(원본 RFP의 핵심 요구). 시연: 회원 등록(보호자 동의 체크) → 상세. 수강내역·PT 잔여세션은 아직 없다.' },
  { id: 'dom-reserve', name: '예약·결제', pill: '구현됨 · Phase 1+2', fig: D.dReserve, rfp: '무료·유료 프로그램 게시 후 예약 · 모바일 결제', view: '실제 PG 대신 <b>모의 결제</b>를 쓰고, 상태 전이는 실물처럼 구현했다', feats: ['회차 예약 · 정원·중복 확인', '모의 결제 승인 후 확정', '24시간 기준 환불·취소', '체크인 처리'], src: '1-7 예약및결제 §6', notes: '예약·결제: 예약 흐름과 24시간 환불 기준(지점별 설정, 기본 24시간). 예약할 때 진행중·정원·중복을 확인한다. 동시성은 구두로: 문서는 DB 행 락을 설계했고, 지금은 동기 실행 mock이라 별도 락 없이 안전하며 DB 전환 시 락을 복원한다. 시연: 회원 예약 → 결제 대기 → 모의결제 → 확정 → 취소(자동 환불). 회원 앱이 미착수라 회원 화면은 회원 계정으로 로그인한 관리자 웹 화면이다.' },
  { id: 'dom-program', name: '강사·프로그램', pill: '구현됨', fig: D.dProgram, rfp: '종목별 강사·프로그램 게시 · 종목·시간대·연령대별 목록', view: '프로그램에 <b>상태</b>를 두어 운영 흐름을 관리한다', feats: ['강사 등록·수정·비활성', '프로그램 등록·수정·종료', '허용된 상태 전이만 통과', '회차 등록 · 연령대·요금 유형'], src: '1-8 강사프로그램게시 §3-2', notes: '강사·프로그램: 프로그램 상태는 준비중 → 진행중 ⇄ 휴강 → 종료로만 바뀌고, 준비중과 휴강에서도 바로 종료할 수 있다. 종료는 되돌릴 수 없다. 예약은 진행중일 때만 가능하다. 시연: 프로그램 등록(자유이용 선택 시 가격·정원 비활성) → 상태 전이.' },
  { id: 'dom-congest', name: '혼잡도관리', pill: '구현됨', fig: D.dCongest, rfp: '시설별 이용 현황·혼잡도 5단계 게시 · 실시간~30분 이내 반영', view: '계측 장비가 없어 <b>관리자가 인원을 보정</b>하면 5단계가 즉시 갱신된다', feats: ['시설 등록·수정', '현재 인원 수동 보정', '5단계 자동 산출', '시설별 혼잡도 카드'], src: '1-9 혼잡도관리 §4', notes: '혼잡도관리: 현재 인원 ÷ 정원의 비율로 5단계(여유·보통·약간 붐빔·붐빔·매우 붐빔)가 정해진다. 계측 장비가 없어 관리자가 인원을 직접 보정하면 단계가 즉시 재계산된다. QR 체크인·5분 자동계산은 앱과 스케줄러가 없어 제외했다. 시연: 보정 전후 카드.' },
...
const SHOTS = {
  ...
  'dom-board': [{ img: B('1a0971d03d90972ccb6bd23707b935e3'), w: 1600, h: 480, url: '/board · 지점 관리자', pts: ['본사 공지와 지점 공지를 한 목록에서 구분', '자기 지점 대상의 글만 열람', '수정은 작성자 본인만'] }],
  'dom-member': [{ img: B('774c549ee1281dfd2d09bc1d86dc8ffe'), w: 1600, h: 900, url: '/members · 지점 관리자', pts: ['지점 관리자가 <b>자기 지점 회원</b>을 등록', '만 19세 미만이면 <b>법정대리인 동의</b>를 확인해야 등록', '회원 상태(활성·휴면)를 목록에서 확인'] }],
  'dom-reserve': [
  ...
  'dom-program': [{ img: B('a3d104aead952097449fe5aef0b73461'), w: 1600, h: 508, url: '/programs · 지점 관리자', pts: ['프로그램 목록과 <b>상태</b>(준비중·진행중·휴강·종료)', '종목·이용방식·가격·강사·시작일 표시', '허용된 상태 전이만 통과'] }],

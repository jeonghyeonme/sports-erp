...
docs/1.spec/1-1~1-9     기능 설계서 — 코드 작업 전 관련 설계서를 먼저 읽을 것
docs/2.decisions/        2-1(ADR: 왜 이 기술을 선택했는지) · 2-2(트러블슈팅) · 2-3(원본 RFP↔설계↔코드 추적표) · 2-4(차별화 전략)
...
번호 체계: 디렉터리 자체에 1(spec)/2(decisions)/3(design)/4(presentation) 번호가 있고, 그 안의 문서는 `1-1`, `2-3`처럼 하위 번호를 붙인다. "1-1문서", "2-3문서" 식으로 지칭한다.
...
- **API는 Prisma가 아니라 `MockDataService`(인메모리)로 동작 중이다.** `apps/api/prisma/schema.prisma`는 설계돼 있지만 실제 컨트롤러는 대부분 mock 데이터를 반환한다. "Prisma 스키마에 있으니 동작한다"고 가정하지 말 것 — 실제 동작 여부는 `docs/2.decisions/2-3_요구사항추적표.md` §2를 확인.
- 대부분의 API가 **조회(GET) 전용**이다. Write API(등록/수정/삭제)는 `permissions/staff/:staffId/role` 정도를 제외하면 거의 없다.
- 1-4(근태관리)·1-7(예약및결제) 도메인은 **설계 문서만 있고 코드가 전혀 없다.**
...
- `Branch`는 단순 매장이 아니라 **위탁계약 현장**이고(`docs/1.spec/1-1_공통설계서.md` §2-1), `Staff`는 지점 소속이 아니라 **본사 소속으로 현장에 파견**되는 구조다(§2-2, `StaffAssignment`). 이 재해석을 모르고 "지점이 직원을 고용한다"는 가정으로 코드를 작성하지 말 것.

## 문서 작업 규칙

- `docs/1.spec/`(1-1~1-9)은 기능 설계, `docs/2.decisions/`(2-1~2-4)는 의사결정·분석 기록, `docs/3.design/`은 UI 디자인 시스템 — 성격에 맞는 폴더에 쓸 것.
- `Branch`, `StaffAssignment` 같은 공유 엔티티의 스키마는 `docs/1.spec/1-1_공통설계서.md`가 단일 진실 공급원이다. 다른 문서에서 필드를 새로 정의하지 말고 1-1문서를 참조할 것.
- 문서 간 상대링크를 쓸 때 실제 파일 위치 기준으로 경로를 맞출 것(`1.spec/`↔`2.decisions/`↔`3.design/`↔`4.presentation/`는 서로 `../` 필요).

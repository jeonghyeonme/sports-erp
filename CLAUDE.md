...
- Write API는 권한관리(로그인/토큰갱신/로그아웃/비밀번호변경/Role전환)·인사정보관리(채용/파견/퇴사)·회원관리(등록/수정/상태전환)·근태관리(체크인/휴가/업무일지)·강사프로그램게시(강사 CRUD·프로그램 등록/수정/종료/상태전이)·게시판(작성/수정/삭제)에 있다. 혼잡도관리·자원문서관리만 여전히 조회 전용이거나 미구현 — 정확한 도메인별 현황은 `docs/2.decisions/60_분석및제안/2-3_요구사항추적표.md` §2-2를 확인.
- **admin-web 프론트엔드가 API를 못 따라간 경우가 있다.** 예: 강사프로그램게시의 지점 현황판 API는 완성돼 있는데 프론트 어디서도 호출하지 않는다. "API가 있으니 화면도 있다"고 가정하지 말 것.
- 1-7(예약및결제)·1-10(자원문서관리) 도메인은 **설계 문서만 있고 코드가 전혀 없다.** 1-4(근태관리)·1-5(게시판)·1-8(강사프로그램게시)는 2026-09-18에 API+화면 모두 구현 완료.
